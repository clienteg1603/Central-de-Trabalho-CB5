...
def first(a,b,label):
    global g
    if a not in g: raise SystemExit('E2B2 ausente: '+label)
    g=g.replace(a,b,1)

def one(a,b,label):
    global g
    n=g.count(a)
    if n!=1: raise SystemExit(f'E2B2 {label}: {n}')
    g=g.replace(a,b,1)

# Primeiras ocorrências pertencem ao posicionamento dos botões Gerar/Juntar.
...
g+='\n# GERAL2_ETAPA2_PAGINAS_V03715\n'
p.write_text(g,encoding='utf-8')
print('GERAL 2 ETAPA 2B2: OK')
