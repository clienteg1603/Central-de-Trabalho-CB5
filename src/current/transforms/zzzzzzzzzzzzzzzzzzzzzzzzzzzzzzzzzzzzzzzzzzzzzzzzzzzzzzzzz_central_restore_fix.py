...
def one(a,b,n):
    global s
    c=s.count(a)
    if c!=1: raise SystemExit(f'{n}: esperado 1, encontrado {c}')
    s=s.replace(a,b,1)
...
s+='\n# CENTRAL_RESTORE_FIX_V02165\n'
for m in ('$script:AppVersion = "0.21.65"','function Invoke-CentralRestorePass','$form.Add_Activated({','CENTRAL_RESTORE_FIX_V02165'):
    if m not in s: raise SystemExit('marcador ausente: '+m)
p.write_text(s,encoding='utf-8')
print('CENTRAL RESTORE FIX: OK')
